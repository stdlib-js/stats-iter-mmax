...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof itermmax, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function throws an error if provided a second argument which is not a positive integer', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			itermmax( randu(), value );
		};
	}
});

tape( 'the function returns an iterator protocol-compliant object which iteratively computes a moving maximum value', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	it = itermmax( array2iterator( values ), 3 );
...
tape( 'if an environment supports `Symbol.iterator` and the provided iterator is iterable, the returned iterator is iterable', function test( t ) {
	...
	itermmax = proxyquire( './../dist/main.js', {
